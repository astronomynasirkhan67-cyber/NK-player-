...
interface VideoDao {
    ...
    @Query("SELECT * FROM videos WHERE isShort = 1 AND durationSeconds > 0 AND durationSeconds <= 90 ORDER BY addedAt DESC")
    fun getShorts(): Flow<List<VideoItem>>

    @Query("SELECT * FROM videos WHERE (isShort = 0 OR durationSeconds > 90) AND durationSeconds > 0 ORDER BY addedAt DESC")
